        ...
        do
          if [ $2 == 'yes' ]; then
            ...
            python main.py --model_type $1 --experiment_description static_use --static_use --no-sampler_use\
             ...
             --aug_method1 ${aug_list[${i}]} --aug_method2 ${aug_list[${j}]} | tee log/$1/$data/$mode/static_use/${aug_list[${i}]}_and_${aug_list[${j}]}/log.txt
          else
            ...
            python main.py --model_type $1 --experiment_description no-static_use --no-sampler_use\
             --dataset $data --device $3 --training_mode $mode --loss_func 'focal'\
             --aug_method1 ${aug_list[${i}]} --aug_method2 ${aug_list[${j}]} | tee log/$1/$data/$mode/no_static/${aug_list[${i}]}_and_${aug_list[${j}]}/log.txt
          fi
        done
      done
    else
